...
def apply_header_based_raw_remap(ws, sheet_name, spec, canonical_keys, raw_last, n_last):
    """Copy formula-critical raw fields to old expected letters when source columns move."""
    headers = ws.range(f"A1:{raw_last}1").value
    if headers is None:
        raise ValueError(f"{sheet_name}: header row is empty")
    if not isinstance(headers, list):
        headers = [headers]
    idx = idx_map_from_headers(headers, spec.columns, list(canonical_keys))

    moved = []
    snapshots = {}
    for key, dest_col in canonical_keys.items():
        src_col = idx[key] + 1
        src_letter = ws.range((1, src_col)).get_address(False, False).rstrip("1")
        values = ws.range((1, src_col), (n_last, src_col)).value
        snapshots[key] = values
        if src_letter.upper() != dest_col:
            moved.append(f"{key}:{src_letter}->{dest_col}")

    for key, dest_col in canonical_keys.items():
        ws.range(f"{dest_col}1:{dest_col}{n_last}").value = snapshots[key]

    if moved:
        log.warning("  %s raw column remap applied: %s", sheet_name, ", ".join(moved))
    else:
        log.info("  %s raw columns already match expected formula positions", sheet_name)
...
def build_output(src, out, master_path, combine_path, workbooks):
    src = Path(src)
    out = Path(out)
    ...
    try:
        ...
        for sheet_name, raw_last, fml_first, fml_last, fml_fn in [
            ...
            canonical_keys = (
                OCH_CANONICAL_KEYS if sheet_name == OCH_SHEET else E2E_CANONICAL_KEYS
            )
            apply_header_based_raw_remap(
                ws,
                sheet_name,
                spec,
                canonical_keys,
