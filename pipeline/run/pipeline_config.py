...
for _sub in ("run", "cpq", ""):
    _folder = _PIPELINE_DIR / _sub if _sub else _PIPELINE_DIR
    _entry = str(_folder.resolve())
...
def default_config_path() -> Path:
    return _PROJECT_ROOT / "config" / "ingest.yml"
...
def build_pipeline_paths(cfg: dict[str, Any], config_path: Path) -> PipelinePaths:
    ...
    return PipelinePaths(
        ...
        fiber_computed_out=computed_dir / f"{week_label.strip()}_NRR_Fiber_computed.xlsx",
