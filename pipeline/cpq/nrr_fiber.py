...
def copy_sheet(source_path: Path, sheet_name: str, target_wb: Workbook) -> None:
    """Copy a raw source sheet into the computed workbook."""
    source_wb = load_workbook(str(source_path), read_only=False, data_only=False, keep_vba=True)
    try:
        if sheet_name not in source_wb.sheetnames:
            log.warning("Source sheet %r not found in %s; skipping copy", sheet_name, source_path.name)
            return
        ...
        log.info("Copied %s sheet into computed workbook", sheet_name)
    finally:
        source_wb.close()
...
def run(config_path: Path) -> int:
    ...
    copy_sheet(cur_file, OCH_SHEET_NAME, wb)
