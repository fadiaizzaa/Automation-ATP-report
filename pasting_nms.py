...
Inputs come from ingest.yml:
  - prepared Card Report: {output_base}/{week_label}/NMS/{week_label}_Card Report.xlsx
  - current NRR: {output_base}/{week_label}/CPQ/{week_label}_Network Resource Statistics.xlsm
...
def formula_omsp_w(row: int, all_fiber_ref: str) -> str:
    return f"=INDEX({all_fiber_ref}$B:$B,MATCH(U{row},{all_fiber_ref}$D:$D,0))"
...
def refresh_omsp_references(
    ...
    if not all_fiber_path.is_file():
        log.warning("All Fiber workbook not found, skipping OMSP V/W refresh: %s", all_fiber_path)
        return
